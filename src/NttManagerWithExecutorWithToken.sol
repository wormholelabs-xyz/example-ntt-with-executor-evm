...
import "example-messaging-executor/evm/src/interfaces/IExecutorWithToken.sol";
...
import "./interfaces/INttManagerWithExecutorWithToken.sol";
import "./interfaces/INttManagerWethUnwrap.sol";

string constant nttManagerWithExecutorWithTokenVersion = "NttManagerWithExecutorWithToken-0.0.1";

/// @title NttManagerWithExecutorWithToken
/// @author Wormhole Project Contributors.
/// @notice The NttManagerWithExecutorWithToken contract is a shim contract that initiates
///         an NTT transfer using the ExecutorWithToken (EQ03) for relaying with token-based payment.
contract NttManagerWithExecutorWithToken is INttManagerWithExecutorWithToken {
    uint16 public immutable chainId;
    IExecutorWithToken public immutable executorWithToken;

    string public constant VERSION = nttManagerWithExecutorWithTokenVersion;

    constructor(uint16 _chainId, address _executorWithToken) {
        assert(_chainId != 0);
        assert(_executorWithToken != address(0));
        chainId = _chainId;
        executorWithToken = IExecutorWithToken(_executorWithToken);
    }

    // ==================== External Interface ===============================================

    /// @inheritdoc INttManagerWithExecutorWithToken
    function transfer(
        address nttManager,
        uint256 amount,
        ...
        // Custody the executor payment token and approve the ExecutorWithToken to spend it.
        uint256 executorTokenAmount = custodyTokens(executorArgs.srcToken, executorArgs.amount);
        _maxApproveIfNeeded(executorArgs.srcToken, address(executorWithToken), executorTokenAmount);
        ...
        executorWithToken.requestExecution{value: executorArgs.value}(
            executorTokenAmount,
            executorArgs.srcToken,
            recipientChain,
